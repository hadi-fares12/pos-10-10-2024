...
async def login(request: Request):
    try:
        ...
        comp_query = (f"Select company.Phone, company.Street, company.City, company.VAT, company.EndTime, company.beginningyear,company.endyear, currencies.Code from company join currencies on company.Currency = currencies.id ")
        ...
        beginning_year = datetime.strptime(comp["beginningyear"], "%d/%m/%Y")
        end_year = datetime.strptime(comp["endyear"], "%d/%m/%Y")

        is_within_year = beginning_year <= current_date <= end_year
        ...
        print("aefe", comp)
        return {"message": "Login successful", "user": user, "comp":comp, "accno":accno ,"is_within_year": is_within_year,"beginningyear": comp["beginningyear"],  "endyear": comp["endyear"]}
