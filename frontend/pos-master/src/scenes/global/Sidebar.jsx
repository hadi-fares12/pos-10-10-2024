...
const Item = ({
  ...
  const beginningyear = localStorage.getItem("beginningyear");
  ...
  const handleClick = () => {
    if (title === t["Manage Items"]) {
      ...
      const beginningYearDate = parseDate(beginningyear);
      const endYearDate = parseDate(endyear);
      ...
      if (currentDate >= beginningYearDate && currentDate <= endYearDate) {
        ...
        setPrRemark("POS section is disabled because it is outside the allowed year range.");
        setAllowDialog(true);
      }
